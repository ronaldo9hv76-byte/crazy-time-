...
with col3:
    ...
    for bonus, sfas, urgency, score in bonus_data:
...
with st.expander("📊 Frequenze Osservate vs Attese (click per aprire)"):
    ...
    for seg in SEGMENTS:
        ...
        freq_rows.append({
            ...
            'Deviazione':  f"{dev:+.0f}%",
            'Gap attuale': sfas_s,
            'Gap atteso':  f"~{exp_gap}",
            'Urgenza':     f"{sfas_s/exp_gap:.1f}x",
        })
...
if len(st.session_state.dealer_history) >= 10:
    ...
    with st.expander("👤 Profilo Dealer Corrente"):
        ...
        dealer_df = pd.DataFrame([
            {'Segmento': s, 'Uscite dealer': dc.get(s,0),
             'Freq %': f"{dc.get(s,0)/len(dh):.1%}"}
            for s in SEGMENTS
        ])
