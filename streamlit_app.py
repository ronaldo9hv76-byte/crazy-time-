...
# --- CONFIGURAZIONE PROFESSIONALE --- st.set_page_config(page_title='Roulette Oracle Pro v4', layout='wide')
# Database Ruota Europea e Settori
WHEEL = [0, 32, 15, 19, 4, 21, 2, 25, 17, 34, 6, 27, 13, 36, 11, 30, 8, 23, 10, 5, 24, 16, 33, 1, 20, 14, 31, 9, 22, 18, 29, 7, 28, 12, 35, 3, 26]
RED_NUMS = {1, 3, 5, 7, 9, 12, 14, 16, 18, 19, 21, 23, 25, 27, 30, 32, 34, 36} SECTORS = {
...
# --- MOTORE MATEMATICO (Dall'architettura v3 PDF) ---
def get_sfasamento(h, target_list):
    for i, val in enumerate(h):
        if val in target_list: return i     return len(h)
def get_analysis_weighted(h):
# [span_1](start_span)Matrice di Markov pesata con decadimento 
esponenziale[span_1](end_span)
    n = len(h)
    ...
    for i in range(len(sec_h)-1, 0, -1):         age = len(sec_h) - 1 - i
weight = math.exp(-0.07 * age) matrix.loc[sec_h[i], sec_h[i-1]] += weight
...
# [span_2](start_span)Test statistico per Bias della ruota[span_2](end_span)
...
# [span_3](start_span)Algoritmo di punteggio composito[span_3](end_span)
    sfas = get_sfasamento(h, SECTORS[sector])
    exp_gap = 1 / EXPECTED_FREQ[sector]
    sfas_score = min(sfas / exp_gap, 2.0) / 2.0
# Bonus se la parità del settore coincide col trend
    parity_bonus = 0.15 if parity_match else 0.0
    return (0.40 * markov_prob + 0.35 * sfas_score + 0.10 * parity_bonus)
# --- INTERFACCIA UTENTE (Basata su v3 PDF e Immagini) --- st.title(' Roulette Oracle Pro v4')
# [span_4](start_span)Telemetria Superiore[span_4](end_span)
...
if chi2_val: c5.metric('Chi2 Bias', f'{chi2_val:.1f}', delta='BIAS' if chi2_val > 7.8 else 'OK')
# Tastiera Numerica
st.write('### Inserisci Risultato')
cols_btn = st.columns(12)
for i in range(1, 37):
    with cols_btn[(i-1)%12]:
        color = "red" if i in RED_NUMS else "black"
        if st.button(str(i), key=f'btn{i}', use_container_width=True):
            if st.session_state.history:
                d = abs(WHEEL.index(st.session_state.history[0]) - WHEEL.index(i))                 st.session_state.distances.insert(0, d)
...
if st.button('0 - ZERO', use_container_width=True, type='primary'):
    st.session_state.history.insert(0, 0); st.session_state.total_spins += 1; st.rerun()
# [span_5](start_span)Cronologia Visiva[span_5](end_span)
if st.session_state.history:
    st.markdown('---')
...
# -[span_6](start_span)[span_7](start_span)-- LOGICA DI DECISIONE (Get Next Bet) [span_6](end_span)[span_7](end_span) ---
if len(st.session_state.history) > 5:
    h = st.session_state.history
    markov, ent_total, ent_recent = get_analysis_weighted(h)
    last_sec = next((s for s, nums in SECTORS.items() if h[0] in nums), 'Voisins')     p_trend, p_strength = get_parity_bias(h)
# Calcolo urgenze best_sector = None
    max_score = -1
    for sec in SECTORS.keys():
        prob = markov.loc[last_sec].get(sec, 0)
# Controllo se i numeri del settore sono coerenti col trend di parità
        ...
        if score > max_score:
            max_score = score
            best_sector = sec
# [span_8](start_span)Box Decisione[span_8](end_span)
    st.markdown('##  PROSSIMA PUNTATA')
    if ent_recent > 2.2:
        st.error(f"ATTENDI: Entropia Alta ({ent_recent:.2f}). Sequenza caotica.")     else:
...
# [span_9](start_span)[span_10](start_span)Dettagli Analisi[span_9](end_span)
[span_10](end_span)
    ...
    with sig3:
        ...
        if st.session_state.distances:
            common_dist = Counter(st.session_state.distances[:10]).most_common(1)
[0]
            st.metric("Salto Dominante", f"+{common_dist[0]}", f" {common_dist[1]*10}% costanza")
# -[span_11](start_span)-- SIDEBAR E CONTROLLI[span_11](end_span) --- st.sidebar.header('Controlli Oracle')
if st.sidebar.button('Cancella Ultimo'):
    if st.session_state.history: st.session_state.history.pop(0); st.rerun() if st.sidebar.button('Cambio Dealer'):
...
p_t, p_s = get_parity_bias(st.session_state.history) st.sidebar.info(f"Trend: {p_t}\nAffidabilità: {p_s:.1%}")
